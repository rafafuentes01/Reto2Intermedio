...
public class CarController : MonoBehaviour
{
    ...
    public float lapTime, bestLapTime, totalTime=0;
    ...
    public float aiAccelerateSpeed = 1f, aiTurnSpeed = .8f, aiReachPointRange = 5f, aiPointVariance = 6f, aiMaxTurn = 15f;
    ...
    public void LapCompleted()
    {
        ...
        else
        {
            if (!isAI)
            {
                ...
                var ts = System.TimeSpan.FromSeconds(bestLapTime);
                UIManager.instance.bestLapTimeText.text = string.Format("{0:00}m{1:00}.{2:000}s", ts.Minutes, ts.Seconds, ts.Milliseconds);
                UIManager.instance.bestLapTimeCopyText.text = string.Format("{0:00}m{1:00}.{2:000}s", ts.Minutes, ts.Seconds, ts.Milliseconds);
